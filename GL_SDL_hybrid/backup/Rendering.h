...
void assign_Quad_Color(object * O, quadrant * Q, id_color * I, int L)
{
    ...
    if (O->vertex_arrays[L])
    {
        for(e = 0; e < 4; e++)
        {
            O->cols_array_[L][c_c++] = I->R;
            O->cols_array_[L][c_c++] = I->G;
            O->cols_array_[L][c_c++] = I->B;
            O->cols_array_[L][c_c++] = I->A;
        }
    }
    L++;
    if (L <= O->subdlevel)
    ...
    if (O->vertex_array)
    {
        ...
        for (p = 0; p < O->polycount; p++)
        {
            ...
            for (e = 0; e < P0->edgecount; e ++)
            {
                if (p == P->index)
                {
                    O->cols_array[c_c++] = I->R;
                    O->cols_array[c_c++] = I->G;
                    O->cols_array[c_c++] = I->B;
                    O->cols_array[c_c++] = I->A;
                }
                else
                {
                    c_c += 4;
...
void put_In_Rectangle_Selection(camera * C, object * O, int * Objects, int Objects_count, int sel_add, int sel_type, dragrectangle * R, int splitview, float CamDist,
                                int draw_uv_view, int cull_Selection, int draw_locators, int L)
{
    GLdouble point[3];
    GLdouble coords[3];
...
int draw_Edge_ID_recursive(camera * C, object * O, edge * E, int L, int l)
{
    ...
    if (E->subdivs == 0 || L == l)
    {
        ...
        if (C->ortho)
        {
            dot = dot_productN_T(&Normal, C->T.aim);
        }
        else
        {
...
int draw_Edge_recursive(object * O, edge * E, int L, int l, camera * C, int wireframe)
{
    if (L > l) return 0;
    ...
    else
    {
        ...
        idx = E->edges[0];
        E0 = &O->edges_[L][idx / ARRAYSIZE][idx % ARRAYSIZE];
        draw_Edge_recursive(O, E0, L, l, C, wireframe);
        idx = E->edges[1];
        E0 = &O->edges_[L][idx / ARRAYSIZE][idx % ARRAYSIZE];
        draw_Edge_recursive(O, E0, L, l, C, wireframe);
    }
        ...
        if (edgedraw && O->subdlevel >= L) // assuming some objects have no geometry on all levels.
        {
            ...
            if (Selection_Mode != OBJES)
            {
                ...
                for (e = 0; e < O->edgecount; e++)
                {
                    ...
                    draw_Edge_recursive(O, E0, 0, L, C, wireframe);
                    idx = E->edges[1];
                    E0 = &O->edges_[0][idx / ARRAYSIZE][idx % ARRAYSIZE];
                    draw_Edge_recursive(O, E0, 0, L, C, wireframe);
                }
...
int draw_Edge(object * O, edge * E, camera * C, int wireframe)
{
    int idx;
...
void render_polys_OnScreen(camera * C, int wireframe, int edgedraw, int vertdraw, int currentObject, int rendermode, int Selection_Mode, int update_colors, int update_uv)
{
    int v, o, p, e, idx;
    object * O;
    id_color * I;
    ...
    for (o = 0; o < C->object_count; o++)
    {
        idx = C->objects[o];
        O = objects[idx];
        glCallList(displayLists[Materials[O->surface].Texture_idx]);

        if (O->vertex_array)
        {
            draw_Fan_Arrays(O, rendermode, update_colors, update_uv);
        }
        else
        {
            for (p = 0; p < O->polycount; p++)
    ...
    for (o = 0; o < C->object_count; o++)
    {
        ...
        if (L > -1 && O->vertex_arrays[L])
        {
            ...
            for (q = 0; q < O->quadcount_[L]; q++)
            {
                ...
                for (e = 0; e < 4; e ++)
                {
                    O->cols_array_[L][c_c++] = R;
                    O->cols_array_[L][c_c++] = G;
                    O->cols_array_[L][c_c++] = B;
                    O->cols_array_[L][c_c++] = A;
                }
            }
        }
    }
...
{
    int o, q, e, idx, idx0, idx1, idx2, L;
    object * O;
    quadrant * Q;
    id_color * I;
...
void fill_in_VertCoords_Fan(camera * C)
{
    int o, p, e, idx;
    object * O;
    polygon * P;
...
void fill_in_VertCoords_quads(camera * C, int l)
{
    int o, q, e, idx, L;
    object * O;
    quadrant * Q;
...
void render_uv_view_(camera * C, int index, int l)
{
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(0, C->h_v_ratio, 0, 1, 1, -1);
...
void render_uv_view(camera * C, int index)
{
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(0, C->h_v_ratio, 0, 1, 1, -1);
...
void render_quads_OnScreen(camera * C, int wireframe, int edgedraw, int vertdraw, int l, int currentObject, int mode, int Selection_Mode, int update_colors, int update_uv)
{
    ...
    for (o = 0; o < C->object_count; o++)
    {
        ...
        if (O->vertex_arrays[L])
        {
            draw_Arrays(O, L, mode, update_colors, update_uv);
        }
