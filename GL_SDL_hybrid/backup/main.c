...
#define TEXT_DIALOG 1
#define SELS_DIALOG 2
#define HIER_DIALOG 3
#define DEFR_DIALOG 4
#define POSE_DIALOG 5
#define OBJ_DIALOG 6
#define IMG_DIALOG 7
...
int init()
{
    ...
    SDL_WM_SetCaption("Trips Code c program", NULL);
    fonts_on = init_fonts();
...
{
    printf("subdivide after Creation\n");
...
int create_Objects()
{
    ...
    result = create_cube(objectIndex, "Cube");
...
void render_Objects(camera * C, int tripsRender, int wireframe, int uv_draw, int rendermode, int level)
{
    if (level < 0)
    {
        if (uv_draw)
        {
            ...
            {
                render_uv_view(C, currentObject);
            }
        }
        ...
        {
            render_polys_OnScreen(C, wireframe, edgedraw, vertdraw, currentObject, rendermode, selection_Mode, UPDATE_COLORS, UPDATE_UV);
        }
    }
    else
    {
        if (uv_draw)
        {
            ...
            {
                render_uv_view_(C, currentObject, level);
            }
        }
        else if (VERTS_ID_RENDER)
        {
        ...
        {
            render_quads_OnScreen(C, wireframe, edgedraw, vertdraw, level, currentObject, rendermode, selection_Mode, UPDATE_COLORS, UPDATE_UV);
        }
    }
}
...
void frame_object(camera * C, int all_Views)
{
    ...
    if (elements_count > 0)
    {
        ...
        if (!DRAW_LOCATORS && len > MIN_SCALE && C->view_minor > MIN_SCALE)
        {
    ...
    else
    {
        ...
        if (all_Views)
        {
            for (c = 0; c < CAMERAS; c ++)
            {
                ...
                C->T.target[0] = O->T.pos[0];
                C->T.target[1] = O->T.pos[1];
                C->T.target[2] = O->T.pos[2];
...
void poly_Render(int tripsRender, int wireframe, int splitview, float CamDist, int Swap, int Level)
{
    ...
    if (!tripsRender)
    {
        if(Level > -1)
        {
            fill_in_VertCoords_quads(Camera, Level);
        }
        else
        {
            fill_in_VertCoords_Fan(Camera);
        }
    }
...
void open_IMG(char * fileName)
{
    ...
    {
#if DEBUG_WITHOUT_IL == 0
                texture = load_ILU_image(fileName);
#else
        ...
        if(texture != NULL)
        {
            ...
            Materials[O->surface].texture = Surf_Text_c;
            memcpy(Text_Names[Surf_Text_c], basename(fileName), strlen(basename(fileName)));
            Surf_Text_c ++;
            Materials[O->surface].Texture_idx = TexFrom_Surface(texture);
...
int list_texts(char ** texts_list, int start, int n)
{
    int s = start;
    int t = 0;
    for (t = 0; t < n; t++)
...
void open_Hierarchys_List()
{
    ...
    if (HierIndex - hier_start >= 0 && HierIndex - hier_start < LISTLENGTH)
    {
...
void open_Textures_List()
{
    ...
    if (Textures_c > 0)
    {
        ...
        if (TextIndex - texts_start >= 0 && TextIndex - texts_start < LISTLENGTH)
        {
            TextList[TextIndex - texts_start].color = UI_BACKL;
            draw_Textures_Dialog("Textures List", screen_height, texts_start, 1);
...
void open_OBJ_List()
{
    ...
    if (dialog_lock)
        ...
        poly_Render(tripsRender, wireframe, splitview, CamDist, 1, subdLevel);
    UPDATE_COLORS = 0;
    draw_Files_Dialog(files_dir, "Open file", screen_height, extension, extensions, ext_count, files_start, 1, 1);
    glDrawBuffer(GL_BACK);
    SDL_GL_SwapBuffers();
    message = 0;
}
...
void handle_UP(int scrollbar)
{
    ...
    else if (dialog_type == TEXT_DIALOG)
    {
        ...
        {
            draw_Textures_List(screen_height, texts_start, 1);
        }
        SDL_GL_SwapBuffers();
    }
    ...
    else
    {
        ...
        if (UPDATE_BACKGROUND)
        {
            draw_Files_Dialog(files_dir, "Open file", screen_height, extension, extensions, ext_count, files_start, 1, 0);
        }
        else
        {
...
{
    printf("init Deformer Objects Binding\n");
...
void add_Pose()
{
    set_Pose_H_Button(0);
    printf("add Pose\n");
    if (!BIND_POSE && posesIndex < POSES - 1)
    {
        if (currentDeformer_Node >= 0 && currentDeformer_Node < deformerIndex)
        {
            add_Pose_To_Deformer(deformers[currentDeformer_Node]);
            PoseIndex ++;
            currentPose ++;
            if (dialog_lock)
                draw_Dialog();
...
{
    if (T->Bone == NULL || (T->Bone && T->Bone->A == T))
    {
        if (T->parent != NULL)
        {
            unparent(T, P);
...
void delete_Transformer(transformer * T)
{
    int c;

    create_Transformers_List();

    transformer * collect_childs[T->childcount];
    int collect_c = 0;
    for (c = 0; c < T->childcount; c ++)
    {
        collect_childs[collect_c ++] = T->childs[c];
    }
    for (c = 0; c < collect_c; c ++)
    {
        if (BIND_POSE)
        {
            remove_Child(collect_childs[c], T, T->parent);
        }
        else
        {
    ...
    remove_Child(T, T->parent, NULL);

    if (T->Deformer != NULL)
    {
        remove_Transformer_From_Deformer(T);
    }
    free(T);
...
void delete_Deformer()
{
    ...
    if (deformerIndex > 0)
    {
        remove_Deformer(deformers[currentDeformer_Node]);
...
//            O->T.rot[1] -= pi60; // profiling
    ...
    if(DRAW_LOCATORS)
    {
        if (transformers[currentLocator] != &World && !BONES_MODE)
        {
            if (ROTATION || SCALE)
            {
                if (BIND_POSE)
                {
                    rotate_bind(transformers[currentLocator]);
                }
                else
                {
                    Update_Objects_Count = 0;
                    if(transformers[currentLocator]->Deformer != NULL)
                    {
                        rotate_Deformer(transformers[currentLocator]);
                    }
                    else
                    {
                        rotate_collect(transformers[currentLocator]);
                        rotate_vertex_groups_D_Init();
                        rotate(transformers[currentLocator], 1);
                    }
                    update_rotate_bounding_box();
                }
            }
            else
            {
                if (BIND_POSE)
                    ...
                    rotate_T(transformers[currentLocator]);
            }
        }
    }
    else
...
//    int rotAxis = 1;
    ...
    SideBar[2] = &open_Textures_List;
    SideBar[3] = &open_Selections_List;
    SideBar[4] = &open_Hierarchys_List;
    SideBar[5] = &open_Deformers_List;
    SideBar[6] = &open_Poses_List;
    SideBar[7] = &collapse_Files;
    SideBar[8] = &open_OBJ_List;
    SideBar[9] = &open_Img_List;
    ...
    Button_h_pose[3].func = &apply_Pose;
...
//                    SDL_GL_SwapBuffers();
            ...
            else if(event.type == SDL_MOUSEBUTTONUP) // scroll up
            {
                ...
                if (drag_rectangle)
                {
                    ...
                    put_In_Rectangle_Selection(Camera, O, selected_objects, selected_object_count, add_selection_mode, selection_Mode,
                                               &Drag_Rectangle, splitview, CamDist, draw_uv_view, cull_Selection, DRAW_LOCATORS, subdLevel);
            ...
            else if(event.type == SDL_MOUSEMOTION)
            {
                ...
                else if (edgeWeights)
                {
                    ...
                    adjust_Selected_Verts_Weights(y);
                    message = -1;
                }
                else if (Camera_screen_lock)
                {
...
//                    }
        ...
        {
            if (DRAW_LOCATORS && !BONES_MODE && BIND_POSE && !Camera_screen_lock && !dialog_lock)
            {
                BONES_MODE = 1;
