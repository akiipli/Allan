...
void adjust_Selected_Verts_Weights(float w)
{
    printf("\radjust %f", w);
    int idx, v, o;
...
int tune_In_Subdivision_Shape_uvtex_(object * O, int L)
{
    ...
    for (e = 0; e < O->uvedcount_[L1]; e ++) // edge verts average surrounding polys and self position
    {
        ...
        if (E->polycount > 1)
        {
            ...
            E->Mu = (Q0->B2.u + Q1->B2.u) / 2.0;
            E->Mv = (Q0->B2.v + Q1->B2.v) / 2.0;
    ...
    for (v = 0; v < O->textcount_[L1]; v ++) // cage verts
    {
        ...
        V0 = &O->verts[UV->vert / ARRAYSIZE][UV->vert % ARRAYSIZE];
...
//                }
        ...
        else
        {
            ...
            U = (Fu + 2.0 * Eu + (UV->uvcount - 3) * UV->u) / edgecount;
            V = (Fv + 2.0 * Ev + (UV->uvcount - 3) * UV->v) / edgecount;

            //if (sqrt((U - UV->u) * (U - UV->u) + (V - UV->v) * (V - UV->v)) < radius)
            //{
                UV->u = U;
                UV->v = V;
            //}
...
void tune_In_Subdivision_Shape_uvtex(object * O)
{
    ...
    for (e = 0; e < O->uvedcount; e ++) // edge verts average surrounding polys and self position
    {
        ...
        if (E->polycount > 1)
        {
            ...
            E->Mu = (P0->B2.u + P1->B2.u) / 2.0;
            E->Mv = (P0->B2.v + P1->B2.v) / 2.0;
...
int tune_In_Subdivision_Shape_transformed_(object * O, int L)
{
    ...
    for (v = 0; v < O->vertcount_[L1]; v ++) // cage verts
    {
        ...
        if (V0->weight != 0 && edgeweight)
        {
            ...
            V->Tx = T_x * V0->weight + Tx * (1 - V0->weight);
            V->Ty = T_y * V0->weight + Ty * (1 - V0->weight);
            V->Tz = T_z * V0->weight + Tz * (1 - V0->weight);
        }
    }
