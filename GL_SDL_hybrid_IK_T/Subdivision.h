...
void *thread_tune_In_Subdivision_Shape_transformed_edges(void *ptr)
{
    targs *p = (targs*)ptr;
    object * O = p->O;
...
void *thread_tune_In_Subdivision_Shape_transformed_verts(void *ptr)
{
    targs *p = (targs*)ptr;
    object * O = p->O;
...
void tune_In_Subdivision_Shape_transformed(object * O)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        ...
        TARGS[t].START = START;
        TARGS[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_tune_In_Subdivision_Shape_transformed_edges, &TARGS[t]);
        START = END;
        if (END >= O->edgecount)
        {
            break;
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        if ((t < N_THREADS - 1) && (START + N_ITEMS < O->vertcount))
        {
            END = START + N_ITEMS;
        }
        else
        ...
        TARGS[t].START = START;
        TARGS[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_tune_In_Subdivision_Shape_transformed_verts, &TARGS[t]);
        START = END;
        if (END >= O->vertcount)
        {
            break;
