...
/*
void *thread_update_bounding_box_for_transformed_Trips(void *ptr)
{
    targs_box *p = (targs_box*)ptr;
    object * O = p->O;
    ...
    for (t = p->START; t < p->END; t++)
    {
        ...
        V1 = &O->verts[T->verts[1] / ARRAYSIZE][T->verts[1] % ARRAYSIZE];
        V2 = &O->verts[T->verts[2] / ARRAYSIZE][T->verts[2] % ARRAYSIZE];
        boxt_3_T(&T->B, V0, V1, V2, p->radius);
    }
    return NULL;
}
...
void update_bounding_box_for_transformed_Trips(object * O, unsigned char radius)
{
    ...
    {
        if ((t < N_THREADS - 1) && (START + N_ITEMS < O->tripcount))
        {
            END = START + N_ITEMS;
        }
        else
        ...
        TARGS_B[t].O = O;
        TARGS_B[t].radius = radius;
        TARGS_B[t].START = START;
        TARGS_B[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_update_bounding_box_for_transformed_Trips, &TARGS_B[t]);
        START = END;
        if (END >= O->tripcount)
        {
            break;
...
void *thread_update_bounding_box_for_transformed_Trips_(void *ptr)
{
    targs_box_L *p = (targs_box_L*)ptr;
    object * O = p->O;
    int L = p->L;
...
void update_bounding_box_for_transformed_Trips_(object * O, int L, unsigned char radius)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        if ((t < N_THREADS - 1) && (START + N_ITEMS < O->tripcount_[L]))
        {
            END = START + N_ITEMS;
        }
        else
        ...
        TARGS_L[t].O = O;
        TARGS_L[t].radius = radius;
        TARGS_L[t].L = L;
        TARGS_L[t].START = START;
        TARGS_L[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_update_bounding_box_for_transformed_Trips_, &TARGS_L[t]);
        START = END;
        if (END >= O->tripcount_[L])
        {
            break;
        }
    }
...
void *thread_update_bounding_box_for_transformed_Edges_(void *ptr)
{
    targs_box_L *p = (targs_box_L*)ptr;
    object * O = p->O;
    int L = p->L;
...
void update_bounding_box_for_transformed_Edges_(object * O, int L, unsigned char radius)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        if ((t < N_THREADS - 1) && (START + N_ITEMS < O->edgecount_[L]))
        {
            END = START + N_ITEMS;
        }
        else
        ...
        TARGS_L[t].START = START;
        TARGS_L[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_update_bounding_box_for_transformed_Edges_, &TARGS_L[t]);
        START = END;
        if (END >= O->edgecount_[L])
        {
            break;
        }
    }
...
/*
void *thread_update_bounding_box_for_transformed_Edges(void *ptr)
{
    targs_box *p = (targs_box*)ptr;
    object * O = p->O;
...
void update_bounding_box_for_transformed_Edges(object * O, unsigned char radius)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        if ((t < N_THREADS - 1) && (START + N_ITEMS < O->edgecount))
        {
            END = START + N_ITEMS;
        }
        else
        ...
        TARGS_B[t].START = START;
        TARGS_B[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_update_bounding_box_for_transformed_Edges, &TARGS_B[t]);
        START = END;
        if (END >= O->edgecount)
        {
            break;
...
void *thread_update_bounding_box_for_transformed_Quads(void *ptr)
{
    targs_box_L *p = (targs_box_L*)ptr;
    object * O = p->O;
    int L = p->L;
    ...
    for (q = p->START; q < p->END; q++)
    {
        ...
        V2 = &O->verts_[L][Q->verts[2] / ARRAYSIZE][Q->verts[2] % ARRAYSIZE];
        V3 = &O->verts_[L][Q->verts[3] / ARRAYSIZE][Q->verts[3] % ARRAYSIZE];
        boxt_4_T(&Q->B, V0, V1, V2, V3, p->radius);
    }
...
void update_bounding_box_for_transformed_Quads(object * O, int L, unsigned char radius)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        if ((t < N_THREADS - 1) && (START + N_ITEMS < O->quadcount_[L]))
        {
            END = START + N_ITEMS;
        }
        else
        ...
        TARGS_L[t].START = START;
        TARGS_L[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_update_bounding_box_for_transformed_Quads, &TARGS_L[t]);
        START = END;
        if (END >= O->quadcount_[L])
        {
            break;
        }
    }
...
/*
void *thread_update_bounding_box_for_transformed_Polys(void *ptr)
{
    targs_box *p = (targs_box*)ptr;
    object * O = p->O;
...
void update_bounding_box_for_transformed_Polys(object * O, unsigned char radius)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        if ((t < N_THREADS - 1) && (START + N_ITEMS < O->polycount))
        {
            END = START + N_ITEMS;
        }
        else
        ...
        TARGS_B[t].START = START;
        TARGS_B[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_update_bounding_box_for_transformed_Polys, &TARGS_B[t]);
        START = END;
        if (END >= O->polycount)
        {
            break;
...
void *thread_generate_vertexNormals_transformed_(void *ptr)
{
...
void generate_vertexNormals_transformed_(object * O, int L)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        if ((t < N_THREADS - 1) && (START + N_ITEMS < O->vertcount_[L]))
        {
            END = START + N_ITEMS;
        }
        else
        ...
        TARGS_T[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_generate_vertexNormals_transformed_, &TARGS_T[t]);
        START = END;
        if (END >= O->vertcount_[L])
        {
            break;
        }
    }
...
/*
void *thread_generate_transformed_edgeNormals(void *ptr)
{
    targs *p = (targs*)ptr;
    object * O = p->O;
...
void generate_transformed_edgeNormals(object * O)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        ...
        TARGS[t].START = START;
        TARGS[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_generate_transformed_edgeNormals, &TARGS[t]);
        START = END;
        if (END >= O->edgecount)
        {
            break;
...
void *thread_generate_transformed_edgeNormals_(void *ptr)
{
    targs_box_T *p = (targs_box_T*)ptr;
    object * O = p->O;
    int L = p->L;
...
void generate_transformed_edgeNormals_(object * O, int L)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        ...
        TARGS_T[t].START = START;
        TARGS_T[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_generate_transformed_edgeNormals_, &TARGS_T[t]);
        START = END;
        if (END >= O->edgecount_[L])
        {
            break;
...
/*
void *thread_generate_transformed_polyNormals(void *ptr)
{
    targs *p = (targs*)ptr;
    object * O = p->O;
...
void generate_transformed_polyNormals(object * O)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        ...
        TARGS[t].START = START;
        TARGS[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_generate_transformed_polyNormals, &TARGS[t]);
        START = END;
        if (END >= O->polycount)
        {
            break;
...
/*
void *thread_update_transformed_Triangles(void *ptr)
{
    targs *p = (targs*)ptr;
    object * O = p->O;
...
void update_transformed_Triangles(object * O)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        ...
        TARGS[t].START = START;
        TARGS[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_update_transformed_Triangles, &TARGS[t]);
        START = END;
        if (END >= O->tripcount)
        {
            break;
...
void *thread_update_transformed_Triangles_level_(void *ptr)
{
    targs_box_T *p = (targs_box_T*)ptr;
    object * O = p->O;
    int L = p->L;
...
void update_transformed_Triangles_level_(object * O, int L)
{
    ...
    for (t = 0; t < N_THREADS; t ++)
    {
        ...
        TARGS_T[t].START = START;
        TARGS_T[t].END = END;
        pthread_create(&THREAD[t], NULL, thread_update_transformed_Triangles_level_, &TARGS_T[t]);
        START = END;
        if (END >= O->quadcount_[L])
        {
            break;
