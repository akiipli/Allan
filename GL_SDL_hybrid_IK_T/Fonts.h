        ...
        while ((ent = readdir(dir)) != NULL)
        {
            ...
            if (isFile(Path))
            {
                ...
                if (strcmp(Extension, ext) == 0)
                {
                    ...
                    {
                        sprintf(out_list[s], "%s", ent->d_name);
                        s ++;
                    }
//                    memcpy(Out_List[s], ent->d_name, strlen(ent->d_name));
//                    Out_List[s][strlen(ent->d_name)] = '\0';
...
void draw_Button_pose_text(const char * text, int width, int height, int index, int colorchange, int frame_it, int x_offset, int x_collapsed)
{
    ...
    if (colorchange)
        ...
        glColor4fv(buttoncolors[UI_BLACK].color);
    if (x_collapsed)
        render_text("+", origin_x - 5, origin_y, font_height);
    else
        render_text("|", origin_x - 5, origin_y, font_height);
	render_text(text, origin_x + 5, origin_y, font_height);
}
...
void draw_Button_path(char * path)
{
	glEnable(GL_TEXTURE_2D);
	...
	render_text(path, origin_x, origin_y, font_height);
}
...
void draw_Files_List(char * path, int s_height, int start, char * ext, int clear_background, int UpdateFilesList)
{
    ...
    {
        s = files_start + LISTLENGTH;
        if (s > files_num) s = files_num;
        for (i = files_start; i < s; i ++)
        {
            draw_Button_text(Out_List[i], d_width, d_height, i - files_start, 1);
        }
    }
...
void draw_Poses_Bottom_Line(int width, int height)
{
    ...
    draw_Button_pose_horizontal("Apply", 3, 1);
...
void draw_Files_Dialog(char * path, const char * text, int s_height, char * ext, char ** extensions, int ext_count, int files_start, int clear_background, int update)
{
    int d_width = DIALOG_WIDTH;
    int d_height = DIALOG_HEIGHT;
    glScissor(SIDEBAR, s_height - d_height + BOTTOM_LINE, d_width, d_height);
